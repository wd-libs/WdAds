...
//    private final Map<String, AppOpenAd> appOpenAdMap = new HashMap<>();
    ...
    public void init(Context context, String sdkKey, AppLovinCallback adCallback) {
        ...
        AppLovinSdkInitializationConfiguration.Builder initConfigBuilder = AppLovinSdkInitializationConfiguration.builder( sdkKey, context );
        ...
        sdk.initialize( initConfigBuilder.build(), appLovinSdkConfiguration -> {
            Log.d(TAG, "init: AppLovinSdk init success");
            if (adCallback != null) {
    ...
    public void init(Context context, String sdkKey, AppLovinCallback adCallback, Boolean enableDebug) {
        String processName = Application.getProcessName();
        String packageName = context.getPackageName();
        if (!packageName.equals(processName)) {
            WebView.setDataDirectorySuffix(processName);
    ...
    public void loadSplashInterstitialAds(final Context context, String id, long timeOut, long timeDelay, AppLovinCallback adListener) {
        ...
        interstitialSplash.setListener(new MaxAdListener() {
            ...
            public void onAdClicked(MaxAd ad) {
                ...
                MKLogEventManager.logClickAdsEvent(context, ad.getAdUnitId());
            }
    ...
    public void onShowSplash(Activity activity, AppLovinCallback adListener) {
        ...
        interstitialSplash.setRevenueListener(ad -> MKLogEventManager.logPaidAdImpression(context,ad, AdType.INTERSTITIAL));
        interstitialSplash.setListener(new MaxAdListener() {
            @Override
            public void onAdLoaded(MaxAd ad) {
            ...
            @Override
            public void onAdClicked(MaxAd ad) {
                MKLogEventManager.logClickAdsEvent(context, interstitialSplash.getAdUnitId());
                if (adListener != null) {
                    adListener.onAdClicked();
                }
                if (disableAdResumeWhenClickAds)
    ...
    public void showInterstitialAdByTimes(final Context context, MaxInterstitialAd interstitialAd, final AdCallback callback, final boolean shouldReloadAds) {
        ...
        interstitialAd.setRevenueListener(ad -> MKLogEventManager.logPaidAdImpression(context,ad, AdType.INTERSTITIAL));
        interstitialAd.setListener(new MaxAdListener() {
            @Override
            public void onAdLoaded(MaxAd ad) {
    ...
    private void loadBanner(final Activity mActivity, String id, final FrameLayout adContainer, final ShimmerFrameLayout containerShimmer) {
        ...
        adView.setRevenueListener(ad -> MKLogEventManager.logPaidAdImpression( mActivity,ad, AdType.BANNER));
    ...
    public void loadNativeAd(Activity activity, ShimmerFrameLayout containerShimmer, FrameLayout nativeAdLayout, String id, int layoutCustomNative) {
        ...
        nativeAdLoader.setRevenueListener(ad -> MKLogEventManager.logPaidAdImpression( activity,ad, AdType.NATIVE));
    ...
    public void showRewardAd(Activity activity, MaxRewardedAd maxRewardedAd, AppLovinCallback callback) {
        if (maxRewardedAd.isReady()) {
            maxRewardedAd.setRevenueListener(ad -> MKLogEventManager.logPaidAdImpression( activity,ad, AdType.REWARDED));
